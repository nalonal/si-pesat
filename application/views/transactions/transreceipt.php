...
<div id="transReceiptToPrint">
    ...
    <div class="row" style='font-weight:bold'>
        ...
        <div class="col-xs-6">Jml</div>
    </div>
    <hr style='margin-top:2px; margin-bottom:0px'>
    <?php $init_total = 0; ?>
    <?php foreach($allTransInfo as $get):?>
        <div class="row">
            <div class="col-xs-6"><?=ellipsize($get['itemName'], 10);?></div>
            <div class="col-xs-6"><?=$get['quantity']; ?></div>
        </div>
        <?php $init_total += $get['quantity'];?>
    <?php endforeach; ?>
    <hr style='margin-top:2px; margin-bottom:0px'>       
    <div class="row">
        <div class="col-xs-12 text-right">
            <b>Total: <?=isset($init_total) ? $init_total.' Item' : 0?></b>
        </div>
    </div>
    <hr style='margin-top:5px; margin-bottom:0px'>
    <div class="row margin-top-5">
        <div class="col-xs-12">
            <b>Untuk: <?=$cust_name?></b><br>
            <b>Admin: <?=$get['pengguna']?></b>
        </div>
    </div>
