...
class Transit extends CI_Controller{
    ...
    public function __construct(){
        ...
        $this->load->model(['item','transitdb']);
    }
    ...
    public function index(){
        $data['pageContent'] = $this->load->view('transit/transit', '', TRUE);
        $data['pageTitle'] = "Barang Transit";
        ...
        $data['allItems'] = $this->transitdb->getAll($orderBy, $orderFormat, $start, $limit);
    ...
    public function kurang(){
        ...
        $allItems = $this->transitdb->getAll($orderBy, $orderFormat, $start, $limit);
    ...
    public function lilt(){
        ...
        $totalItems = $this->db->count_all('barang_transit');
        ...
        $config = $this->genlib->setPaginationConfig($totalItems, "transit/lilt", $limit, ['onclick'=>'return lilt(this.href);']);
        ...
        $data['sn'] = $start+1;
        $data['cum_total'] = $this->transitdb->getItemsCumTotal();
        
        $json['itemsListTable'] = $this->load->view('transit/transitlisttable', $data, TRUE);//get view with populated items table
    ...
    public function add(){
        ...
        if($this->form_validation->run() !== FALSE){
            ...
            $insertedId = $this->transitdb->add(set_value('itemName'), set_value('itemQuantity'), set_value('itemPrice'), set_value('itemKategori'), set_value('itemDescription'), set_value('itemCode'),set_value('barCode'), set_value('itemMinimal'), set_value('itemSatuan'), set_value('itemLokasi'));
    ...
    */
    ...
    public function gcoandqty(){
        ...
        if($itemCode){
            $item_info = $this->transitdb->getItemInfo(['code'=>$itemCode], ['quantity', 'unitPrice', 'description']);
    ...
    public function updatestock(){
        ...
        if($this->form_validation->run() !== FALSE){
            ...
            $updated = $updateType === "deficit" 
                    ? 
                $this->transitdb->deficit($itemId, $qty, $desc) 
                    : 
                $this->transitdb->newstock($itemId, $qty, $desc);
    ...
    */
    ...
    public function edit(){
        ...
        if($this->form_validation->run() !== FALSE){
            ...
            //update item in db
            $updated = $this->transitdb->edit($itemId, $itemName, $itemDesc, $itemPrice, $itemKategori, $itemQuantity, $itemMinimal, $barCode,$itemSatuan,$itemLokasi);
    ...
    // PINDAH ITEM SUBMIT
     public function pindahitem(){
        ...
        if($this->form_validation->run() !== FALSE){
            ...
            //update item in db
            $updated = $this->transitdb->pindahitem($itemId, $itemName, $itemDesc, $itemPrice, $itemKategori, $itemQuantity, $itemMinimal, $barCode,$itemSatuan,$itemLokasi, $itemCode);
    ...
    */
    ...
    public function delete(){
        ...
        if($item_id){
            $this->db->where('id', $item_id)->delete('barang_transit');
