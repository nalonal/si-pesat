...
class Transactions extends CI_Controller{
    ...
    public function __construct(){
        ...
        $this->load->model(['transitdb','transaction', 'item']);
    }
    ...
    public function index(){
        ...
        $transData['semuaUnit'] = $this->item->semuaUnit('satuan', 'ASC');

        //$transData['transititems'] = $this->item->transitgetActiveItems('name', 'ASC');//get items with at least one qty left, to be used when doing a new transaction
    ...
    */
    ...
    private function insertTrToDb($arrOfItemsDetails, $cust_name){
        ...
        foreach($arrOfItemsDetails as $get){
            ...
            $jenis = $get->jenis;//unit price of item in loop
            $harga = $get->unitPrice;

            $transId = $this->transaction->add($itemName, $itemCode, "", $qtySold, $jenis, $cust_name, $ref, $harga);
            
            $allTransInfo[$transId] = ['itemName'=>$itemName, 'quantity'=>$qtySold];
            
            //update item quantity in db by removing the quantity bought
            //function header: decrementItem($itemId, $numberToRemove)
            if($jenis == 'transit'){
                $this->item->decrementTransit($itemCode, $qtySold);
            }
            else{
    ...
    private function transitinsertTrToDb($arrOfItemsDetails, $cust_name){
        $this->load->model(['transitdb']);
        $allTransInfo = [];//to hold info of all items' in transaction
        ...
        foreach($arrOfItemsDetails as $get){
            ...
            $transId = $this->transaction->transitadd($itemName, $itemCode, "", $qtySold, $unitPrice, $totalPrice, $cust_name, $ref);
    ...
    */
    ...
    public function report($from_date, $to_date=''){        
        ...
        $data['allTransactions'] = $this->transaction->getDateRange($pilihUnit,$from_date, $to_date);
        
        $this->load->view('transactions/transReport', $data);
    }


    public function laporan($from_date, $to_date='',$pilihUnit){        
        //get all transactions from db ranging from $from_date to $to_date
        //$data['pilihUnit'] = $pilihUnit;
        $unit = explode("%20", $pilihUnit);
        $hasilUnit = implode(" ", $unit);
        if($hasilUnit == "Semua Unit"){
            $hasilUnit = "*";
        }
        $data['dari'] = $pilihUnit;
        $data['from'] = $from_date;
        $data['to'] = $to_date ? $to_date : date('Y-m-d');
        $data['allTransactions'] = $this->transaction->getDateRange($hasilUnit,$from_date, $to_date);
        $this->load->view('transactions/transReport', $data);
    }
}
