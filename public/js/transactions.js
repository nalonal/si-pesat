...
$(document).ready(function(){
    ...
    // $("#coba").on('change', '#custName', function(e){
    //     e.preventDefault();
    //     alert('hai');
    // });
    ...
    $("#transSearchDate").keyup(function(){
        var value = $(this).val();
        
        if(value){
            $.ajax({
                url: appRoot+"search/transsearchDate",
                type: "get",
                data: {v:value},
                success: function(returnedData){
                    $("#transListTable").html(returnedData.transTable);
    ...
    $("#transitappendClonedDivHere").on("change", ".itemTransQty", function(e){
        e.preventDefault();
    ...
    //handle untuk data transit
        $("#transitconfirmSaleOrder").click(function(){
        var custName = $("#custName").val();
        if(!custName){
            !custName ? $("#custNameErr").text("Mohon diisi") : "";
            return;
        ...
        for(var i = 0; i < selectedItemNodeLength; i++){
            ...
            var jenisbarangNode = selectedItemNode[i].parentNode.parentNode.children[1].children[1];
            var definitifNode = selectedItemNode[i].parentNode.parentNode.children[2].children[1];
            var transitNode = selectedItemNode[i].parentNode.parentNode.children[3].children[1];
            var permintaanNode = selectedItemNode[i].parentNode.parentNode.children[5].children[1];
            var hargaNode = selectedItemNode[i].parentNode.parentNode.children[4].children[1];
            //var totalPriceNode = selectedItemNode[i].parentNode.parentNode.children[6].children[1];

                //get values
            var jenisbarang = jenisbarangNode[jenisbarangNode.selectedIndex].value;
            var definitif = parseInt(definitifNode.innerHTML);
            var transit = parseInt(transitNode.innerHTML);
            var permintaan = parseFloat(permintaanNode.value);
            var hargaBarang = parseFloat(hargaNode.innerHTML);

            //var totalPrice = parseFloat(totalPriceNode.innerHTML);
            //var expectedTotPrice = +(unitPrice*qty).toFixed(2);
            if(jenisbarang === 'transit'){
                var tersedia = parseInt(transit);
                var jenis = "transit";
            }
            if(jenisbarang === 'definitif'){
                var tersedia = parseInt(definitif);  
                var jenis = "definitif";              
            }
                //validate data
            if((tersedia === 0) || (tersedia < permintaan)){
                // totalPriceNode.style.backgroundColor = expectedTotPrice !== totalPrice ? "red" : "";
                permintaanNode.style.backgroundColor = (tersedia === 0) || (tersedia < permintaan) ? "red" : "";    
                return;
            }

            else{
                    //if all is well, remove all error bg color
                permintaanNode.style.backgroundColor = "";
                                        
                    //then prepare data to add to array of items' info
                var itemInfo = {_iC:itemCode, qty:permintaan, jenis:jenis, unitPrice:hargaBarang};
        ...
        displayFlashMsg("Sedang memproses transaksi", spinnerClass, "", "");

        $.ajax({
            url: appRoot+"transactions/transitnso_",
            method: "post",
            data: {_aoi:_aoi,cn:custName},
            success:function(returnedData){
                if(returnedData.status === 1){
    ...
    $("#transitcancelSaleOrder").click(function(e){
        e.preventDefault();
        
        transitresetSalesTransForm();
    });
    ...
    $("#showTransForm").click(function(){
        ...
        $("#showTransForm").hide();
        $("#showTransitForm").hide();
        $("#hideTransForm").show();
    });



    ///UNTUK SHOW/HIDE TRANSAKSI TRANSIT//
    $("#showTransitForm").click(function(){
        $("#newTransitDiv").toggleClass('collapse');
        $("#showTransForm").hide();
        $("#showTransitForm").hide();
        $("#hideTransitForm").show();
    });
    ...
    $("#hideTransForm").click(function(){
        ...
        resetSalesTransForm();
        $("#showTransForm").show();
        $("#showTransitForm").show();
        $("#hideTransForm").hide();

    });

    $("#hideTransitForm").click(function(){
        $("#newTransitDiv").toggleClass('collapse');
    ...
    $("#clickToGen").click(function(e){
        ...
        var pilihUnit = $("#pilihUnit :selected").text();
        var fromDate = $("#transFrom").val();
        var toDate = $("#transTo").val();
        
        if(!fromDate){
            $("#transFromErr").html("Pilih dari Tanggal");
        ...
        //window.open(appRoot+"transactions/report/"+pilihUnit+"/"+fromDate+"/"+toDate, 'Print', strWindowFeatures);
        window.open(appRoot+"transactions/laporan/"+fromDate+"/"+toDate+"/"+pilihUnit, 'Print', strWindowFeatures);
    });
});
...
/**
 ////////////////////////////////////////////////////////////////////////
 ////////////////////////////////////////////////////////////////////////
 * KOMPLEKS CODINGAN UNTUK FUNGSI : UNTUK HASIL SELECT PADA TRANSAKSI
 * TANGGAL PEMBUATAN              : 16 SEPTEMBER
 * KETERANGAN                     : -
 ////////////////////////////////////////////////////////////////////////
 ////////////////////////////////////////////////////////////////////////
 **/
...
function selectedItem(selectedNode){
    if(selectedNode){
        ...
        var itemTransit = selectedNode.parentNode.parentNode.children[3].children[1];
        var itemDefinitif = selectedNode.parentNode.parentNode.children[2].children[1];
        var hargaNode = selectedNode.parentNode.parentNode.children[4].children[1];
        ...
        $.ajax({
            ...
            success: function(returnedData){
                if(returnedData.status === 1){
                    ...
                    itemTransit.innerHTML = returnedData.transit;
                    if(returnedData.transit === 0 || !returnedData.transit){
                        itemTransit.innerHTML = "0";
                    }
                    itemDefinitif.innerHTML = returnedData.availQty;
                    hargaNode.innerHTML = returnedData.unitPrice;
...
//data transit
function transitselectedItem(selectedNode){
    if(selectedNode){
        //get the elements of the selected item's avail qty and unit price
        var itemAvailQtyElem = selectedNode.parentNode.parentNode.children[1].children[1];
        var itemUnitPriceElem = selectedNode.parentNode.parentNode.children[2].children[1];
...
function transitresetSalesTransForm(){
    document.getElementById('salesTransForm').reset();
    ...
    //remove all appended lists
    $("#transitappendClonedDivHere").html("");
}
